...
export function RotatableWheel({ onActiveProjectChange }: RotatableWheelProps) {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center py-6 select-none w-full">
      ...
      <div className="absolute top-2 font-mono text-[15px] text-signal-red/50 uppercase tracking-widest flex gap-8 z-10">
                ...
                >
                  ...
                  <div className="relative z-10 flex h-full flex-col justify-between p-8">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-lg text-signal-yellow bg-black/70 px-3 py-1 border border-signal-yellow/30 tracking-widest">
                        NODE // {project.id.slice(0, 4).toUpperCase()}
                      </span>
                      <span className="font-mono text-lg font-bold text-white/50 bg-black/60 px-2 py-0.5">
                        [ 0{i + 1} ]
                      </span>
                    </div>
                    ...
                    <div className="max-w-md">
                      ...
                      <p className="mt-2 font-mono text-lg text-signal-yellow uppercase tracking-wider bg-black/60 px-2 py-1 inline-block border-l-2 border-signal-yellow">
                        {project.tagline}
                      </p>
                    </div>
                  </div>
        ...
        <div className="h-full w-full bg-void-raised p-6" style={{ clipPath: "polygon(29px 0, 100% 0, 100% calc(100% - 29px), calc(100% - 29px) 100%, 0 100%, 0 29px)" }}>
          <header className="mb-4">
            <div className="flex items-center gap-3">
              <h4 className="font-display text-lg tracking-wider text-signal-yellow uppercase">
                {activeProject.title}
              </h4>
            ...
            <p className="mt-1 font-mono text-lg text-signal-yellow/70">
              {activeProject.tagline}
            </p>
          </header>

          <p className="mb-4 font-mono text-lg leading-relaxed text-signal-yellow/80">
            {activeProject.description}
          </p>

          <h5 className="font-display text-lg uppercase tracking-widest text-signal-red mb-2">
            Project Highlights
          </h5>
          <ul className="mb-5 space-y-1.5 font-mono text-lg text-signal-yellow/80">
